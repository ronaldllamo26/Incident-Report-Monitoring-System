...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        ...
        if ($name && $email && $password) {
            ...
            if ($check->fetch()) { header('Location: /irms/portal/admin/users.php?error='.urlencode('Ginagamit na ang email.')); exit; }
            $pdo->prepare("INSERT INTO users (name,email,password,role,phone,agency) VALUES (?,?,?,?,?,?)")
                ->execute([$name,$email,password_hash($password,PASSWORD_BCRYPT),$role,$phone,$agency?:null]);
...
$users=$pdo->query("SELECT u.*,(SELECT COUNT(*) FROM incidents WHERE reporter_id=u.id) AS report_count FROM users u ORDER BY u.created_at DESC")->fetchAll();
?>
<!DOCTYPE html>
<html lang="fil">
<head>
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover align-middle mb-0">
                            <thead class="table-light">
                                <tr>
                                    <th class="ps-3 small">#</th><th class="small">Name</th><th class="small">Email</th>
                                    <th class="small">Agency</th><th class="small">Role</th><th class="small">Reports</th>
                                    <th class="small">Status</th><th class="small">Joined</th><th class="small">Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="small fw-medium"><?= htmlspecialchars($u['name']) ?></td>
                                    <td class="small text-muted"><?= htmlspecialchars($u['email']) ?></td>
                                    <td class="small">
                                        <?php if ($u['agency']): ?>
                                            <span class="badge bg-info text-dark small"><?= htmlspecialchars($u['agency']) ?></span>
                        ...
                        <?= csrf_field() ?>
                                    ...
                                    <td><span class="badge bg-<?= $u['is_active']?'success':'secondary' ?> small"><?= $u['is_active']?'Active':'Inactive' ?></span></td>
